...
bool _isNXPUCODEAESTag = false;
bool _isNMV2DTag = false;
bool _isM775Tag = false;
    ...
    if(0 < trd.data.len)
    {
      ...
      if (_isM775Tag)
      {
          if (0x8000 == trd.data.len)
          {
              ...
              // Incase of embedded tag op, more than 1 tag will be read. The tag read data of each tag can give error
              // if occurs. Prints the error for each tag read.
              printf("Embedded tagOp failed:%s\n ", TMR_strerr(reader, ret));
          }
          else
          {
              //Response Data contains Tag's Shortened TID (8 bytes) and Tag's Response (8 bytes)
              TMR_bytesToHex(trd.data.list, (trd.data.len / 8), dataStr);
              printf("Response Data: %s\n", dataStr);

              //Tag's Shortened TID
              TMR_bytesToHex(trd.data.list, 8, dataStr);
              printf("Tag's Shortened TID: %s\n", dataStr);
              //Tag's Response
              TMR_bytesToHex(trd.data.list + 8, 8, dataStr);
              printf("Tag's Response: %s\n", dataStr);
          }
      }
      else
      {
          ...
          TMR_bytesToHex(trd.data.list + 10, dataLen - 10, dataStr);
          printf("Raw data(%d): %s\n", (dataLen - 10), dataStr);
      }
    }
  }
	...
	/* Read Plan */
	{
		...
		**/ 
...

		{
			...
			TMR_TagOp_GEN2_NXP_M775Authentication M775;
			...
			//Enable flag _isM775Tag for Authentication for M775 tag
			if (_isM775Tag)
			{
				ret = TMR_TagOp_init_GEN2_NXP_AES_M775Authentication(&M775);
				checkerr(rp, ret, 1, "initializing M775 authentication");

				authenticate.M775Auth = M775;
				authenticate.type = M775_AUTHENTICATION;

				ret = TMR_TagOp_init_GEN2_NXP_AES_Authenticate(&newtagop, &authenticate);
				checkerr(rp, ret, 1, "initializing Authenticate");

				/*Enable both SenRep and IncRepLen(default)*/
				//newtagop.u.gen2.u.custom.u.nxp.u.authenticate.M775Auth.Authentication = 0x03;

				/*Enable SenRep only*/
				//newtagop.u.gen2.u.custom.u.nxp.u.authenticate.M775Auth.Authentication = 0x01;
				...
				checkerr(rp, ret, 1, "executing Authenticate tagop");
				{
					...
					//Tag's Shortened TID
					TMR_bytesToHex(dataList.list, 8, dataStr);
					printf("Tag's Shortened TID: %s\n", dataStr);
					//Tag's Response
					TMR_bytesToHex(dataList.list + 8, 8, dataStr);
					printf("Tag's Response: %s\n", dataStr);
				}
